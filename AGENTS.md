...
id=agent.plans mode=* role=* phase=all tags=plans,local,privacy ports=* svc=* pre=writing_remediation_or_implementation_plan act=save_under_docs/local/plans/<name>.md verify=path_is_gitignored next=share_with_user block=commit_plan_to_tracked_paths refs=docs/local/plans/,.gitignore
