...
def update_source(name: str, *, limit: int, dry_run: bool) -> dict:
    ...
    with SessionLocal() as db:
        ...
        if result.error:
            ...
            db.commit()
            return report
        report["db"] = service.upsert_records(result.records, commit=False)
...
def main() -> int:
    ...
    for source in sources:
        report["sources"].append(update_source(source, limit=args.limit, dry_run=args.dry_run))
